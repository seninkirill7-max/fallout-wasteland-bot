...
db = sqlite3.connect("wasteland.db")cursor = db.cursor()
...
def create_player(user_id):cursor.execute("INSERT OR IGNORE INTO players (user_id) VALUES (?)",(user_id,))db.commit()

def get_player(user_id):cursor.execute("SELECT * FROM players WHERE user_id=?",(user_id,))return cursor.fetchone()

def add_xp(user_id, amount):cursor.execute("UPDATE players SET xp=xp+? WHERE user_id=?",(amount, user_id))db.commit()

def set_xp(user_id, xp):cursor.execute("UPDATE players SET xp=? WHERE user_id=?",(xp, user_id))db.commit()

def set_level(user_id, level):cursor.execute("UPDATE players SET level=? WHERE user_id=?",(level, user_id))db.commit()
